...
제1조(목적)
이 약관은 HIP-Coder(이하 "회사"라 한다)가 제공하는 Thing Q(이하 "서비스"라 한다)의 이용과 관련하여 회사와 이용자 간의 권리, 의무 및 책임 사항을 규정함을 목적으로 한다.

제2조(정의)
① "서비스"란 회사가 제공하는 웹사이트, 모바일 애플리케이션 등을 통해 이용자가 이용할 수 있는 모든 서비스 및 콘텐츠를 의미한다.
② "이용자"란 이 약관에 동의하고 서비스를 이용하는 자를 의미한다.
③ "회원"이란 서비스를 이용하기 위해 회사와 이용 계약을 체결하고 회사가 제공하는 서비스를 이용하는 자를 의미한다.
④ "이용계약"이란 이 약관에 동의하고 서비스를 이용하기 위해 회사와 이용자가 체결하는 계약을 의미한다.
⑤ "아이디"란 회원이 서비스 이용을 위해 설정한 고유한 문자 및 숫자의 조합을 의미한다.
⑥ "비밀번호"란 회원이 설정한 아이디와 일치하는 이용자임을 확인하기 위해 설정한 문자 및 숫자의 조합을 의미한다.

제3조(약관의 효력 및 변경)
① 이 약관은 서비스를 이용하고자 하는 모든 이용자에게 그 효력을 발생한다.
② 회사는 필요 시 이 약관을 변경할 수 있으며, 변경된 약관은 서비스 내 공지 또는 기타 방법으로 이용자에게 고지된다.
③ 변경된 약관은 공지된 일자로부터 효력이 발생한다.
④ 이용자는 변경된 약관에 동의하지 않는 경우 서비스 이용을 중단하거나 이용계약을 해지할 수 있다.

제4조(서비스의 제공)
① 회사는 서비스의 내용과 범위를 지속적으로 개선하며, 이용자가 서비스를 이용하는 데 불편함이 없도록 최선의 노력을 다한다.
② 서비스는 일정 기간 동안 운영될 수 있으며, 회사는 이를 사전 고지하고 변경하거나 중단할 수 있다.
③ 회사는 서비스 이용에 있어 발생할 수 있는 기술적 문제나 서비스 중단에 대해서 사전에 고지할 수 있으나, 긴급한 상황에서는 사후 고지가 가능하다.

제5조(이용 계약의 체결)
① 이용자가 본 약관에 동의하고 서비스를 이용하려면 회원 가입 절차를 완료해야 한다.
② 회사는 회원 가입 신청에 대해 승낙할 의무가 없다. 다만, 회사가 회원 가입을 승인한 경우 그 결과에 따라 서비스를 제공한다.
③ 이용자는 서비스 이용을 위해 제공하는 개인정보가 사실임을 보증하며, 허위 정보를 제공한 경우 이용계약이 해지될 수 있다.

제6조(서비스 이용)
① 이용자는 본 약관에 따라 서비스를 이용할 수 있으며, 회사는 이용자에게 서비스를 제공할 의무가 있다.
② 이용자는 서비스 이용에 있어 관련 법령을 준수해야 하며, 불법적인 행위를 하여서는 안 된다.
③ 회사는 서비스 이용과 관련하여 발생하는 제반 문제에 대해 이용자에게 적절한 안내를 제공한다.

제7조(이용자의 의무)
① 이용자는 본 약관을 준수하고, 서비스 이용에 있어 타인의 권리를 침해하거나 불법적인 행위를 해서는 안 된다.
② 이용자는 서비스를 이용할 때 발생하는 모든 행위에 대해 책임을 진다.
③ 이용자는 자신의 아이디와 비밀번호를 안전하게 관리해야 하며, 이를 타인에게 노출하거나 부정 사용되어서는 안 된다.

제8조(회사의 의무)
① 회사는 서비스를 제공하는 데 있어 이용자의 개인정보를 보호하고, 관련 법령에 따라 이를 처리한다.
② 회사는 서비스의 품질을 유지하기 위해 지속적인 점검 및 보수 작업을 시행한다.
③ 회사는 서비스를 제공하는 과정에서 발생할 수 있는 이용자의 불편 사항에 대해 적절한 해결을 위해 노력한다.

제9조(개인정보 보호)
① 회사는 이용자의 개인정보를 보호하기 위해 최선을 다하며, 개인정보의 수집, 이용, 보관, 처리 등에 관한 사항은 개인정보 처리방침에 따른다.
② 이용자는 언제든지 자신의 개인정보를 열람하고 수정할 수 있다.
③ 회사는 이용자의 개인정보를 제3자에게 제공하거나 공유할 때 법적 근거가 있거나 이용자의 동의를 받은 경우에만 이를 처리한다.

제10조(서비스의 중단 및 종료)
① 회사는 서비스 제공을 위한 설비 점검, 보수, 기술적 문제 등으로 인해 일시적으로 서비스가 중단될 수 있다.
② 회사는 서비스의 중단 및 종료에 대해 이용자에게 사전 고지하고, 필요한 경우 이용자에게 적절한 보상 방안을 제시할 수 있다.
③ 이용자는 서비스의 중단 및 종료로 인해 발생한 손해에 대해 회사에게 책임을 묻지 않는다.

제11조(저작권)
① 서비스에 포함된 모든 콘텐츠(문서, 이미지, 동영상, 소프트웨어 등)의 저작권은 회사 또는 해당 저작권자가 보유한다.
② 이용자는 서비스를 이용하면서 회사의 저작권을 침해해서는 안 된다.
③ 이용자는 서비스 내에서 제공되는 콘텐츠를 무단으로 복제하거나 배포할 수 없다.

제12조(면책사항)
① 회사는 천재지변, 불가항력적 사유 또는 제3자의 부주의 등으로 인한 서비스 이용의 중단, 오류, 손해에 대해 책임을 지지 않는다.
② 이용자가 서비스를 이용하면서 발생한 손해에 대해서는 이용자가 직접 해결해야 하며, 회사는 이에 대해 책임을 지지 않는다.

제13조(분쟁 해결)
① 본 약관에 관한 분쟁은 회사와 이용자가 협의하여 해결한다.
② 분쟁이 해결되지 않을 경우, 회사의 본사 소재지 법원을 관할 법원으로 한다.

부칙
① 이 약관은 2024년 11월 18일부터 시행한다.
